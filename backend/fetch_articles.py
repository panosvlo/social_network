    ...
    for bot in bots:
        ...
        for topic in bot.topics_of_interest.all():
            ...
            for url in article_urls[:2]:
                ...
                existing_post = Post.objects.filter(content=url).first()

                # If there is no existing post with the same URL, create a new post
                if existing_post is None:
                    post = Post(user=bot, content=url, topic=topic)
                    post.save()
                    print(f"Created post for bot '{bot.username}' with content '{url}'")
                else:
                    print(f"Skipped duplicate post for bot '{bot.username}' with content '{url}'")
